...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`catadmin`@`%` SQL SECURITY DEFINER VIEW `view_sm_emp` AS select `a`.`emp_code` AS `emp_code`,`a`.`emp_name` AS `emp_name`,`a`.`emp_pos_desc` AS `emp_pos_desc`,`a`.`emp_pos_short` AS `emp_pos_short`,`a`.`emp_email` AS `emp_email`,`a`.`org_code` AS `org_code`,`a`.`default_auth` AS `default_auth`,`a`.`emp_admin_code` AS `emp_admin_code`,`b`.`org_name` AS `org_name`,`b`.`org_short` AS `org_short`,`a`.`emp_actived` AS `emp_actived`,`a`.`actived` AS `actived` from (`tbl_sm_emp` `a` join `tbl_sm_org` `b` on((`a`.`org_code` = `b`.`org_code`))) */;
