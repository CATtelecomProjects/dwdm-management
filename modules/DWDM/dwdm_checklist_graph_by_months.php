...
// Get month for select box
 $sql_months = "SELECT DISTINCT
					 DATE_FORMAT(check_date, '%m') AS check_month
					FROM tbl_dwdm_checklist_detail
					WHERE YEAR(check_date) = $years
					ORDER BY check_month DESC";
...
echo "<td width='50%'>";
        ...
        echo "<select name='select_month' id='select_month'>";
          ...
          for($i=0;$i<count($rs_months);$i++){ //ThaiMonthFull as $key => $value){
			  $sel = $rs_months[$i]['check_month'] == $_GET['months'] ? ' selected ' : '';
			  
			echo "<option value='".$rs_months[$i]['check_month']."' $sel>".$ThaiMonthFull[$rs_months[$i]['check_month']]." ".($years+543)."</option>";	  
		  }
		 echo    "</select>";

echo "</td>";
echo "<td width='50%' align='right'>";
echo $div_graph = "&nbsp;<span class='btnGraphY''> <button>กราฟรายปี</button></span>";
echo "</tr>";
echo "</table>";
