...
if($action == "MAPPING"){     
		...
		//delete mapping user by position automation
		$sqlDelMapping = "DELETE FROM tbl_sm_mapping_user
									WHERE emp_code IN(SELECT
														emp_code
													  FROM tbl_sm_emp
													  WHERE org_code = '$org_code'
														  AND emp_admin_code ='$admin_code'); ";
		$rsDelMapping = $db->GetAll($sqlDelMapping);
		...
		if(count($rs_get_emp)>0){ // if have member
			...
			/*
				...
				*/
				...
				$rsChkCount  = $db->GetAll($sqlChkCount);																	
				$ActiveStatus = 	count($rsChkCount) > 0 ? 'Y' : 'N';
				
				$sqlUpdateActived = "UPDATE tbl_sm_emp SET actived = '$ActiveStatus' WHERE emp_code = '$emp_code' ";		
				$rsUpdateActived = $db->Execute($sqlUpdateActived);																		
