...
$arrData = explode('|',$_GET['org_data']);

$org_code = $arrData[0];
$org_name = $arrData[1];
$org_short = $arrData[2];
$admin_code = $arrData[3];
$emp_pos_short = $arrData[4];
$emp_pos_desc = $arrData[5];
					  ...
					  icons: {
					   primary: 'ui-icon-zoomin'
						 ...
						 $.setDialog("mapping-user",780,640);
							...
							$.get('./modules/'+setModule+'/dwdm_sm_mapping_user_list_report.php?'+$.now(),		
			...
			var $sharedPortal = $("input[id^='chk_"+module_name+"']").filter(':first');
			 ...
			 if (countChkBox >0) {
				...
				var row_1st = $sharedPortal.attr('rel');		
						...
						$sharedPortal.prop('checked',true); 
						$('#row_'+row_1st).addClass("ui-state-active");
					
					var countChkBox = $("input[id^='chk_"+module_name+"']").filter(':checked').length;    	    
					//alert(countChkBox);
					
			 }else{
					 $('#row_'+row_1st).removeClass("ui-state-active");
...
/*	$('table#tbl_mapping tr ').click(function() {
 ...
 <table width="100%" border="0" cellspacing="1" cellpadding="1" id="tbl_mapping">
   <tr class="ui-state-focus">
  	  <th align="left" height="20"><img src="./images/2_step.png" align="absmiddle" > <?=$org_code." : ".$org_name." (".$org_short.") &#8250; ".$emp_pos_desc." (".$emp_pos_short.")"?></th>
	  </tr>
      </table>
<div id="accordion">
  <?php
  $sql_emp = "SELECT
							...
							FROM tbl_sm_module a
							  ...
							  LEFT JOIN tbl_sm_mapping_position c
								ON (b.rep_group_id = c.rep_group_id
									AND c.org_code = '$org_code' AND c.admin_code = '$admin_code')
...
?>
  ...
  <div>
    ...
    <td colspan="2" align="left" class="ui-state-highlight"><label><input name="chk_<?=$module_name?>" id="<?=$module_name?>"  type="checkbox" value=""  class="check_all">Select All</label>  </td>
    </tr>
 <?php
  	$sql_report_group = "SELECT
									...
									FROM tbl_sm_report_group a
									  LEFT JOIN tbl_sm_mapping_position b
										ON (a.rep_group_id = b.rep_group_id
											AND b.org_code = '$org_code'
											AND b.admin_code = '$admin_code')
  ...
  ?>
    ...
    <td width="78%" align="left"><label><div><input name="chk_data[]" id="chk_<?=$module_name?>_<?=$rep_group_id?>" type="checkbox" value="<?=$rep_group_id?>" ref="<?=$module_name?>" rel="<?=$module_name?>_<?=$emp_code?>_<?=$rep_group_id?>" <?=$chk;?>> <?=$rep_group_name?></div></label></td>
    <td width="22%" align="right"><a href="javascript:void(0)" id="<?=$rep_group_id?>" rel="<?=$rep_group_name?>" class="btn_report">Detail</a></td>
    </tr>
 <?php 
	} // End Report Loop
?>
...
<label for="admin_code"></label>
<input type="hidden" name="admin_code" id="admin_code" value="<?=$admin_code?>">
