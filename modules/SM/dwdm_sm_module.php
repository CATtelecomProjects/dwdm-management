...
// List User Group
$sql_list = "SELECT * FROM tbl_sm_module  ORDER BY update_time DESC ";
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="display compact" id="<?= $tbl->id; ?>">
    <thead>
        <tr>
            ...
            <th width="26%" align="center">Module Name</th>
            <th width="43%" align="center"> Module Description</th>
            <th width="22%" align="center">Update Time</th>
        </tr>
    </thead>
    <tbody>
        <?php for ($i = 0; $i < count($rs_list); $i++) { ?>
            <tr>
                <td align="center"> <input type="radio" name="selID" id="selID_<?= $rs_list[$i]['module_name'] ?>" value="<?= $rs_list[$i]['module_name'] ?>" <?= $i == 0 ? 'checked' : '' ?>/></td>
...
<input type="hidden" name="hidRadio" id="hidRadio" value="<?= $rs_list[0]['module_name'] ?>" />
